(ns medsecretary.actor
  "MedicalSecretaryActor — the ISCO-08 3344 medical secretarial
  scheduling/filing coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  scheduling/filing operation request (intake → advise → govern →
  ...
  The unconditional invariant: the MedicalSecretaryAdvisor can never
  directly commit a scheduling/filing operation the
  MedicalSecretaryGovernor refuses — every commit-record! call is gated
  behind `:decide`. In particular the governor's scope exclusion
  (never finalize disclosure of a patient's medical record, never
  provide clinical judgment, never authorize a prescription/refill) is
  a hard, permanent block that this graph structurally cannot bypass:
  those proposals always route to `:hold`, never to `:commit` or even
  `:request-approval`."
...
(defn build-graph
  "Build a compiled MedicalSecretaryActor graph. `store` implements
  `medsecretary.store/Store`. `advisor` implements
  `medsecretary.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:provider-id (:provider-id request)
                                    :op (:op proposal)
                                    :appointment-id (:appointment-id proposal)
                                    :payload proposal}]
